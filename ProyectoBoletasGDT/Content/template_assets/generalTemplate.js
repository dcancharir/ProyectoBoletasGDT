...
function messageResponse(obj) {
    ...
    switch (opciones.type) {
        case 'warning':
            //opciones.type = null;
            toastr.warning(opciones.text, opciones.title, { timeOut: opciones.delay });
            break;
        case 'success':
            toastr.success(opciones.text, opciones.title, { timeOut: opciones.delay });
            break;
        case 'error':
            toastr.error(opciones.text, opciones.title, { timeOut: opciones.delay });
            break;
    };
}
...
function messageConfirmation(obj) {
    ...
    //    callBackSCCerrarcomplete: null,
    //    background: '#263544',
    //};
    //var opciones = $.extend({}, defaults, obj);
    //Swal.fire({
    //    title: opciones.title,
    //    text: opciones.content,
    //    icon: opciones.icon,
    ...
    //    background: opciones.background,
    //}).then((result) => {
    ...
    //    }
    //    else {
    //        if (opciones.callBackSCCerrarcomplete != null) {
    //            opciones.callBackSCCerrarcomplete();
    //        }
    //    }
    //})
    // $.confirm({
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (datos.length > 0) {
                ...
                if (opciones.select2) {
                    $("#" + opciones.select).select2({ width: "100%", placeholder: opciones.placeholder, multiple: opciones.multiple, dropdownParent: opciones.dropdownParent });
                }
...
function simpleSelect(obj) {
    ...
    // let obj="_objetoSelect";
    let datos = opciones.data;
    $("#" + opciones.select).html("");
    $("#" + opciones.select).append('<option value="">' + opciones.placeholder + '</option>');
    if (datos.length > 0) {
        if (opciones.allOption) {
